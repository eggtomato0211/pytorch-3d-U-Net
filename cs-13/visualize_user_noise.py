...
noisy_file = Path(r"D:/nosaka/data/3d-holography_output/Train_user_noisy_test/128images_10plots_fixed_randomFalse_NumberFrom1025_user_noisy.h5")

print(f"📂 比較対象:")
print(f"  元データ: {original_file.name}")
print(f"  ユーザー修正後ノイズ: {noisy_file.name}")
...
fig.suptitle(f"User's Noise Implementation: Original vs Noisy\n(Scatter + Cloud Haze + Sensor Noise)", 
...
# ノイズデータのMIP
axes[1, 0].imshow(np.max(noisy_raw, axis=0), cmap='hot')
axes[1, 0].set_title('User Noisy - MIP (Top View)', fontsize=11)
axes[1, 0].axis('off')

axes[1, 1].imshow(np.max(noisy_raw, axis=1), cmap='hot')
axes[1, 1].set_title('User Noisy - MIP (Side View 1)', fontsize=11)
axes[1, 1].axis('off')

axes[1, 2].imshow(np.max(noisy_raw, axis=2), cmap='hot')
axes[1, 2].set_title('User Noisy - MIP (Side View 2)', fontsize=11)
axes[1, 2].axis('off')

plt.tight_layout()
mip_output_path = Path(r"c:\Users\Owner\mizusaki\pytorch-3dunet\cs-13\user_noise_comparison_mip.png")
...
fig3d.suptitle(f"3D Visualization: User Noise Implementation Comparison", fontsize=14, fontweight='bold')

# しきい値以上のポイントを抽出
threshold_orig = np.percentile(original_raw, 99.5)
threshold_comp = np.percentile(noisy_raw, 99.5)

z_orig, y_orig, x_orig = np.where(original_raw > threshold_orig)
z_comp, y_comp, x_comp = np.where(noisy_raw > threshold_comp)

intensity_orig = original_raw[z_orig, y_orig, x_orig]
intensity_comp = noisy_raw[z_comp, y_comp, x_comp]
...
ax2.set_title('User Noisy Data', fontsize=12)
ax2.set_xlim(0, 128); ax2.set_ylim(0, 128); ax2.set_zlim(0, 128)

plt.tight_layout()
scatter_output_path = Path(r"c:\Users\Owner\mizusaki\pytorch-3dunet\cs-13\user_noise_comparison_3d.png")
